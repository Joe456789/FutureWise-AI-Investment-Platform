...
# 3. 流量保護：加入 6.1 秒安全延遲，完美繞過 FinMind 每小時 600 次限制。
...
def fetch_single_stock_chips(ticker, start_date_str):
    """
    ★ 破案改良版：接收精確的「開始日期字串」，保證涵蓋需要的交易日
    """
    ...
    params = {
        ...
        "data_id": ticker, "start_date": start_date_str, "end_date": end_date, "token": FINMIND_TOKEN
    }
    try:
...
def run_v8_5_engine():
    ...
    for i, code in enumerate(tickers):
        name = name_map[code]
        print(f"[{i+1}/{len(tickers)}] 同步 {code} {name} (準備回溯 {TARGET_SAVE_DAYS} 天)...", end="\r")
        try:
            ...
            # ==========================================
            # ★ 破案機制 1：動態計算 FinMind 抓取的起點日期
            # 從 yfinance 剛抓下來的交易日中，精確找到 60 天前的確切日期
            # 這樣就不用管農曆過年到底放了幾天假了！
            # ==========================================
            if len(df) >= TARGET_SAVE_DAYS:
                start_date_val = df['交易日期'].iloc[-TARGET_SAVE_DAYS]
            else:
                start_date_val = df['交易日期'].iloc[0]
            
            start_date_str = start_date_val.strftime('%Y-%m-%d')
            
            # 向 FinMind 要求這個日期之後的所有資料
            df_chips = fetch_single_stock_chips(code, start_date_str)
            ...
            # ==========================================
            # ★ 破案機制 2：存檔筆數必須對齊 TARGET_SAVE_DAYS
            # 把 tail() 裡面的數字直接綁定常數，保證抓幾天就存幾天！
            # ==========================================
    ...
    print(f"\n🎉 掛機任務圓滿達成！全市場過去 {TARGET_SAVE_DAYS} 個真實交易日的空洞與籌碼皆已對齊修復。")
